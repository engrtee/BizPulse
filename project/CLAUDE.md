...
- Scheduler: node-cron for daily 7pm WAT trigger
...
├── services/
│   ├── whatsapp.js     (send/receive)
│   ├── gemini.js       (AI parsing + recommendations)
│   ├── email.js        (build + send emails)
...
├── jobs/
│   └── dailySummary.js (7pm WAT cron)
├── utils/
│   ├── formatter.js    (₦ formatting, dates)
...
consecutive_days_streak, -- retention mechanic
...
total_ever_received, unit_price,
...
- Daily entry saving to PostgreSQL
- Streak tracking updating correctly
- Dashboard showing revenue, profit, top expense, customers
- Summary page with AI recommendation, expense breakdown,
...
**Known issues to fix:**
- Margin calculation displaying incorrectly
- AI recommendation is generic — not using actual user data
- Month comparison showing ₦0/NO when no previous month data
- Mobile navigation not yet implemented (sidebar on phone)
- "How It Works" section showing on Home for returning users
- Home page missing: AI insight preview, last 5 days activity
- Summary page too busy — needs progressive disclosure
